...
def create_menu(db: Session, menu_data: MenuCreateSchema) -> None:
    ...
    if is_existing:
        raise HTTPException(status_code=status.HTTP_409_CONFLICT, detail="이미 등록된 메뉴입니다.")
    new_item = MenuModel(**menu_data.dict())
    db.add(new_item)
    db.commit()

def get_menu_list(db: Session, skip: int = 0, limit: int = 100) -> List[MenuResponseSchema]:
    menu_items = (db.query(MenuModel).filter(MenuModel.is_deleted == False).offset(skip).limit(limit).all())
